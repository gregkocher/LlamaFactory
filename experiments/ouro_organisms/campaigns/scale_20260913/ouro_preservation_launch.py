...
p=argparse.ArgumentParser();p.add_argument('role',choices=['trainer','qualification']);p.add_argument('--review-acceptance',required=True);a=p.parse_args()
...
script+=f'test -f {base}/data_preservation/manifest.json\n'
script+=f'{py} {code}/make_scale_config.py --config {base}/{run}.yaml --run-id {run} --scope r64_all --dataset-dir {base}/data_preservation --dataset {arm} --output-dir /workspace/scale_runs/{run} --token-budget 100000000 --batch-size 8 --accumulation 2 --early-steps 1,50,122,244,400,800,1221,2442 --save-steps 100000 --checkpoint-minutes 20 --wall-hours 4\n'
